...
///
/// Consumed by `crate::algorithms::rsa::rsa_private_op_and_check_blinded`
/// to sample the blinding factor `r` fed to `rsa_private_op_blinded`.
/// Plain code span rather than an intra-doc link because the target
/// is feature-gated and would break `cargo doc --no-default-features`.
